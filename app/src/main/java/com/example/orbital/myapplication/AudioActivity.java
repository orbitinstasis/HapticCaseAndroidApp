...
/**
 * Created by Orbital on 24/07/2015.
 */
   ...
   */
            ...
            new SerialInputOutputManager.Listener() {
                ...
                public void onNewData(final byte[] data) {
                    ...
                    AudioActivity.this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            AudioActivity.this.updateReceivedData(data);
                        }
                    });
                }
            };
    ...
    private void AudioInputThread() {
        checkButThread = new Thread(){
            public void run() {
                fDebug.setText("Frequency: "+ String.valueOf(fr));
                fSliderDebug.setText("Slider: " + String.valueOf(slidePos));
                for (int i = 0; i<ROWS; i++) {
                    for (int j = 0; j < COLS; j++) {
                        if (padCell[i][j] != oldPadCell[i][j]) {
                            oldPadCell[i][j] = padCell[i][j];
                            if (padCell[i][j] > 0) {
                                slidePos = (j * 0.06);//* 1/15
                                amp = (int)map(padCell[i][j], 0, TOP_PAD_FORCE, 0, 10000);
                            } else
                                slidePos = 0;
                        }
                    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        fDebug = (TextView) findViewById(R.id.debugFR);
        fSliderDebug = (TextView) findViewById(R.id.debugSlider);
